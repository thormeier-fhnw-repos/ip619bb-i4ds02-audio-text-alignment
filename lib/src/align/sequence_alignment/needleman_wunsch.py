...
def needleman_wunsch(a: List, b: List, match_reward: int, mismatch_penalty: int, gap_penalty: int) -> Tuple[
    List, List, int]:
    """
    ...
    """
    score_matrix = create_score_matrix(a, b, match_reward, mismatch_penalty, gap_penalty, True, False)

    return walk_matrix(score_matrix, len(a), len(b), a, b, match_reward, mismatch_penalty, gap_penalty)
