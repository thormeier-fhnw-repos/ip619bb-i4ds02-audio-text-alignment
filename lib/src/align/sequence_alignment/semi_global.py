...
def semi_global(a: List, b: List, match_reward: int, mismatch_penalty: int, gap_penalty: int,
    ...
    """
    score_matrix = create_score_matrix(a, b, match_reward, mismatch_penalty, gap_penalty, compare_function, True, True)

    return walk_matrix(score_matrix, len(a), len(b), a, b, match_reward, mismatch_penalty, gap_penalty, compare_function, True)
